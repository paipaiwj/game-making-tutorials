...
L05 渲染工具：
  ...
  - 两者文件名同名（.svg / .png），lesson-05.html 不需要改
"""
...
# SVG 渲染缩放倍率（1 = 原始尺寸；2 = 视网膜清晰度）
RENDER_SCALE = 2
OUT_DIR = r"D:\\bangong\\unity\\vrc\\ass\\ceshi\\img"
...
def add_text(dwg, text, x, y, *, fill=TEXT, size=14, weight="normal",
    ...
    """在 SVG 中添加文字。anchor: start | middle | end"""
    style = {
...
def save_svg_and_png(dwg, name, scale=RENDER_SCALE):
    """把 svgwrite 的 dwg 落盘成同名 .svg，并渲染成 .png。"""
    ...
    # 用 resvg 渲染（纯文本 SVG 字符串 → PNG bytes）
